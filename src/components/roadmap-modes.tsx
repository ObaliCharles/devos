    ...
    <div className="grid gap-3 sm:grid-cols-2">
      {roadmaps.map((r) => (
        ...
        >
          ...
          {r.summary && <p className="text-body mt-2 line-clamp-2 text-ui">{r.summary}</p>}

          <div className="mt-auto pt-4">
            {r.active ? (
              <span
                className="flex items-center gap-1.5 text-ui font-medium"
...
function GenerateMode({ configured }: { configured: boolean }) {
  ...
  if (!configured) {
    return (
      <p
        className="rounded-[var(--radius-tile)] p-3 text-ui"
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <div className="lg:col-span-2">
        {error && (
          <p
            className="mb-3 rounded-[var(--radius-tile)] p-3 text-ui"
