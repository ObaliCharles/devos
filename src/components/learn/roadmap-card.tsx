...
export function RoadmapCard({
  ...
  return (
    <div className="card group flex flex-col p-5">
      ...
      <p className="text-body mt-1 line-clamp-2 flex-1 text-ui">
        {r.summary || "A structured path with lessons, projects and milestones."}
      </p>

      <ul className="mt-4 flex flex-wrap gap-x-4 gap-y-1.5 text-micro" style={{ color: "var(--text-muted)" }}>
      ...
      <button
        onClick={follow}
        disabled={pending}
        className="mt-4 flex items-center justify-center gap-1.5 rounded-[var(--radius-control)] py-2 text-ui font-medium transition-colors disabled:opacity-60"
        style={{
