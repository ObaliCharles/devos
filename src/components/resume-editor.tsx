        ...
        <ListSection
          ...
          render={(item, update) => (
            <>
              ...
              <textarea
                className="input mt-2 min-h-[70px] resize-y text-ui"
        ...
        <div className="card p-4">
          ...
          {ats && ats.findings.length > 0 && (
            <ul className="mt-3 flex flex-col gap-1.5">
              {ats.findings.slice(0, 5).map((finding) => (
                <li key={finding} className="text-micro" style={{ color: "var(--text-muted)" }}>• {finding}</li>
              ))}
            </ul>
          )}
          {ats && ats.strengths.length > 0 && (
            <ul className="mt-2 flex flex-col gap-1">
              {ats.strengths.map((s) => (
                <li key={s} className="flex items-center gap-1 text-micro" style={{ color: "var(--success)" }}>
                  <Check size={11} /> {s}
                </li>
              ))}
            </ul>
          )}
          <p className="mt-3 text-micro" style={{ color: "var(--text-faint)" }}>Recomputed on every save.</p>
        </div>
  ...
  return (
    <section className="card p-5">
      <div className="flex items-center justify-between">
        ...
        <button className="btn btn-ghost h-8 px-3 text-ui" onClick={() => onChange([...items, { ...blank }])}>
          <Plus size={14} /> Add
        </button>
      </div>
