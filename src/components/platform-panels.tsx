    ...
    <div className="flex flex-col gap-4">
      {anyUnread && (
        <div className="flex justify-end">
          <button className="btn btn-ghost h-8 px-3 text-ui" onClick={() => run(() => markAllNotificationsRead())} disabled={pending}>
            ...
            const body = (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-micro" style={{ color: "var(--text-faint)" }}>{relativeDate(n.at as string)}</p>
                </div>
              </div>
            );
            return (
          ...
          {resources.map((r) => (
            <div key={String(r.id)} className="card flex items-start justify-between p-4">
              <div className="min-w-0">
                <span className="text-micro" style={{ color: "var(--text-faint)" }}>{String(r.kind)}</span>
          ...
          <ul className="flex flex-col gap-2">
            {tickets.map((t) => (
              <li key={String(t.id)} className="card flex items-center justify-between p-3 text-sm">
                <span>{String(t.subject)} <span className="text-micro" style={{ color: "var(--text-faint)" }}>· {String(t.kind)}</span></span>
                <span className="text-micro" style={{ color: "var(--text-muted)" }}>{String(t.status)}</span>
              </li>
            ))}
          </ul>
        </section>
