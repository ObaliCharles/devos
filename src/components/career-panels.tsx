          ...
          {active.map((status) => {
            ...
            return (
              <section key={status} className="flex w-[240px] shrink-0 flex-col rounded-[var(--radius-card)] border p-2.5" style={{ borderColor: "var(--border)", background: "var(--surface)" }}>
                <p className="px-1.5 py-1 text-ui font-semibold capitalize">
                ...
                <div className="mt-1 flex flex-col gap-2">
                  {items.map((a) => (
                    <article key={String(a.id)} className="rounded-[var(--radius-card)] border p-2.5" style={{ borderColor: "var(--border)", background: "var(--surface-2)" }}>
                      <p className="text-ui font-medium">{String(a.position)}</p>
                      <p className="text-micro" style={{ color: "var(--text-muted)" }}>{String(a.company)}</p>
                      <div className="mt-2 flex items-center gap-1">
                        <select
                          className="input h-7 flex-1 py-0 text-micro"
                  ...
                  {items.length === 0 && <p className="px-1.5 py-2 text-micro" style={{ color: "var(--text-faint)" }}>Empty</p>}
                </div>
              </section>
            );
          })}
            ...
            <li key={String(i.id)} className="card p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  <select className="input h-8 w-28 py-0 text-micro" value={String(i.outcome)} onChange={(e) => run(() => updateInterview(String(i.id), { outcome: e.target.value }))} aria-label="Outcome">
          ...
          <ul className="mt-3 flex flex-col gap-1.5">
            {clients.map((c) => (
              <li key={String(c.id)} className="flex items-center justify-between text-sm">
                <span>{String(c.name)} <span className="text-micro" style={{ color: statusColor(String(c.status)) }}>{String(c.status)}</span></span>
          ...
          {projects.map((pr) => (
            <li key={String(pr.id)} className="flex items-center justify-between rounded-[var(--radius-card)] border p-3" style={{ borderColor: "var(--border)" }}>
              <div>
                ...
                <p className="text-micro" style={{ color: "var(--text-faint)" }}>{String(pr.status)}{pr.liveUrl ? " · has live URL" : ""}</p>
              </div>
              <span className="text-micro" style={{ color: pr.visibility === "public" ? "var(--success)" : "var(--text-faint)" }}>
