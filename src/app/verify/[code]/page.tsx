    ...
    <main className="grid min-h-screen place-items-center px-5 py-16">
      <div className="w-full max-w-[520px]">
        <Link href="/" className="mb-8 flex items-center justify-center gap-2.5" aria-label="DeveloperOS home">
          <LogoMark size={22} />
          <span className="text-ui font-semibold tracking-[-0.03em]">
        ...
        <div className="card p-6 sm:p-8">
          {result.status === "valid" ? (
            ...
            <>
              ...
              <p className="text-body mt-6 text-ui">
        ...
        <p className="text-meta mt-6 text-center text-micro">
...
function Field({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    ...
    >
      <dt className="text-ui" style={{ color: "var(--text-faint)" }}>
        {label}
      </dt>
      <dd className={`text-ui font-medium ${mono ? "num" : ""}`} style={{ margin: 0 }}>
        {value}
      </dd>
    </div>
  );
